...
class MultiParticlePtCorrelations:
    ...
    def _transverse_momentum_correlations_event_num_denom(self, particle_list_event: List[Particle]) -> None:
        """
    ...
    def _compute_numerator_denominator_all_events(self, particle_list_all_events: List[List[Particle]]) -> None:
        """
    ...
    def mean_pt_correlations(self, particle_list_all_events: List[List[Particle]], compute_error: bool=True, delete_fraction: float=0.4, number_samples: int=100, seed: int=42) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        """
    ...
    def _kappa_cumulant(self, C: np.ndarray, k: int) -> float:
        ...
        if k == 1:
            ...
            kappa = (C[3] - 4.*C[2]*C[0] - 3.*C[1]**2 + 12.*C[1]*C[0]**2 
                        - 6.*C[0]**4)
        elif k == 5:
    ...
    def mean_pt_cumulants(self, particle_list_all_events: List[List[Particle]], compute_error: bool=True, delete_fraction: float=0.4, number_samples: int=100, seed: int=42) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        """
        Computes the mean transverse momentum cumulants for each order k
        ...
        for order in range(self.max_order):
            ...
            combined_data = np.empty((dataN.shape[0], dataN.shape[1] + dataD.shape[1]), dtype=dataN.dtype)
            ...
            if compute_error:
                jackknife = Jackknife(delete_fraction, number_samples, seed)
                kappa_store_error[order] = jackknife.compute_jackknife_estimates(combined_data, function=self._compute_mean_pt_cumulants, k=order)
