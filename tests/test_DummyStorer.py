...
from sparkx.Storer.DummyStorer import DummyStorer
...
def test_dummy_storer_initialization():
    storer = DummyStorer(particle_object_list)
    assert storer.particle_list_ == particle_object_list

def test_create_loader():
    storer = DummyStorer(particle_object_list)
    storer.create_loader(particle_object_list)
    assert isinstance(storer.loader_, DummyLoader)
...
def test_particle_list_single_event():
    storer = DummyStorer([particle_object_list[0]])
