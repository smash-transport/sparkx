...
def test_dummy_loader_initialization():
    loader = DummyLoader(particle_list)
...
def test_load_with_filters():
    ...
    expected_particles = [[p for p in particles if p.charge == 1.0] for particles in particle_list]
...
def test_load_with_unknown_keyword():
    loader = DummyLoader(particle_list)
    with pytest.raises(ValueError, match='Unknown keyword argument used in constructor'):
        loader.load(unknown_key='value')

def test_load_with_invalid_events_tuple():
    loader = DummyLoader(particle_list)
    with pytest.raises(ValueError, match='First value of event number tuple must be smaller than second value'):
...
def test_apply_kwargs_filters():
    ...
    loader = DummyLoader(particle_list)
    filtered_events = [loader._DummyLoader__apply_kwargs_filters([event], filters)[0] for event in particle_list]
    expected_particles = [[p for p in particles if p.charge == 1.0] for particles in particle_list]
