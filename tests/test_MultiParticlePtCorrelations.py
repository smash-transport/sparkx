...
def test_MultiParticlePtCorrelations_mean_pt_correlations(mpc_instance):
    ...
    with pytest.raises(TypeError):
        mpc_instance.mean_pt_correlations(particle_list_all_events=particle_list, 
                                          compute_error="true", 
...
def test_MultiParticlePtCorrelations_mean_pt_cumulants(mpc_instance):
    ...
    with pytest.raises(TypeError):
        mpc_instance.mean_pt_cumulants(particle_list_all_events=particle_list, 
                                       compute_error="true", 
                                       delete_fraction=delete_fraction, 
                                       number_samples=number_samples, 
                                       seed=seed)
...
def test_MultiParticlePtCorrelations_mean_pt_correlations_and_cumulants_physics(mpc_instance):
    ...
    for _ in range(10): # create 10 events
        ...
        for _ in range(100): # create 100 particles with the same pt
            p_array = np.array([0,2114,11,2.01351754,1.30688601,-0.422958786,-0.512249773])
