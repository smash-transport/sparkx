...
def test_EventPlaneFlow_integrated_flow_errors(test_data):
    with pytest.raises(TypeError):
        ...
        EventPlaneFlow(
            n=2,
            weight="pt2",
            pseudorapidity_gap=0.1).integrated_flow(
            test_data,
            test_data,
            self_corr="not_a_bool")
...
def test_EventPlaneFlow_differential_flow_errors(test_data):
    ...
    with pytest.raises(TypeError):
        ...
        EventPlaneFlow(
            n=2,
            weight="pt2",
            pseudorapidity_gap=0.1).differential_flow(
            test_data,
            "not_a_list",
            "pt",
            test_data)
    ...
    with pytest.raises(TypeError):
        ...
        EventPlaneFlow(
            n=2, weight="pt2", pseudorapidity_gap=0.1).differential_flow(
            test_data, [
                0, 1, 2], 123, test_data)
...
def test_LeeYangZeroFlow_differential_flow_errors(test_data):
    ...
    with pytest.raises(TypeError):
        lyz_flow.differential_flow(
            test_data, [
                0, 1, 2], "pt", poi_pdg="not_a_list_or_ndarray")
...
def test_QCumulant_flow_differential_flow_errors(test_data):
    ...
    with pytest.raises(ValueError):
        QCumulantFlow(
            n=2, k=3, imaginary='zero').differential_flow(
            test_data, [
                0, 1, 2], "pt", poi_pdg=[
                1, 2])
