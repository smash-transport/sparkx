...
def oscar_old_extended_file_path():
    # Assuming your test file is in the same directory as test_files/
    return os.path.join(
        os.path.dirname(__file__), "test_files", "particle_lists_extended_old.oscar"
    )
...
def test_num_output_per_event(tmp_path, oscar_old_extended_file_path):
    ...
    oscar_old_extended = Oscar(oscar_old_extended_file_path)
    assert (
        oscar_old_extended.num_output_per_event() == num_output_oscar_old_extended
    ).all()
...
def test_filter_in_oscar(tmp_path):
    ...
    # Event 1: 6 proton spectators, 12 proton participants and 18 pi0
    # participants
    for i in range(6):
...
def test_filter_in_oscar_constructor(tmp_path):
    ...
    # which are selected with keyword arguments while initializing the Oscar
    # object.
    ...
    oscar_empty = Oscar(
        oscar_file, filters={"charged_particles": True, "uncharged_particles": True}
    )
