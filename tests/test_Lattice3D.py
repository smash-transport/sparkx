...
def test_get_index_nearest_neighbor_within_range(sample_lattice):
    values = np.linspace(0, 1, 10)
    assert sample_lattice._Lattice3D__get_index_nearest_neighbor(
        0.3, values) == 3
...
def test_extract_slice_to_csv(sample_lattice, tmp_path):
    ...
    with open(filename, 'r') as f:
        ...
        expected_rows = [['5.000000000000000000e+00',
    ...
    for i in range(20):
        for j in range(20):
            for k in range(20):
                ...
                assert np.isclose(
                    new_lattice.get_value_by_index(
                        i, j, k), lattice.interpolate_value(
                        posx, posy, posz))
...
def test_add_particle_data(
        ...
        for j in range(10):
            for k in range(10):
                if (i == 0 and j == 0 and k == 0):
                    assert np.isclose(
                        sample_lattice.get_value_by_index(
                            i, j, k), 1. / sample_lattice.cell_volume_)
                if (i == 5 and j == 5 and k == 5):
                    assert np.isclose(
                        sample_lattice.get_value_by_index(
                            i, j, k), 2. / sample_lattice.cell_volume_)
        ...
        for j in range(10):
            for k in range(10):
                ...
                if (i == 5 and j == 5 and k == 5):
                    assert np.isclose(
                        sample_lattice.get_value_by_index(
                            i, j, k), 4. / sample_lattice.cell_volume_)
    ...
    assert np.isclose(
        sample_lattice.get_value_nearest_neighbor(
            0.444,
            0.444,
            0.444) /
        sample_lattice.get_value_nearest_neighbor(
    ...
    deltap1 = (-0.11111 - 0.11111 - 2. * 0.22222) / \
        (gamma * particle_list_center[0].mass)
    deltap2 = (-0.22222 - 0.33333 + 2. * 0.22222) / \
        (gamma * particle_list_center[0].mass)
    ratio = np.exp(-0.5 * (deltas1**2 + deltap1**2) * (1 / 1e-1)**2) / \
