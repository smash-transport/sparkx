...
def test_is_valid_index(sample_lattice):
    assert sample_lattice._Lattice3D__is_valid_index(1, 2, 3)
...
def test_within_range(sample_lattice):
    assert sample_lattice._Lattice3D__is_within_range(0.3, 0, 1)
    assert sample_lattice._Lattice3D__is_within_range(2, 0, 1) == False


def test_interpolate_value_nearest(sample_lattice):
    sample_lattice.set_value_nearest_neighbor(0.3, 0.4, 0.49, 5.0)
    assert sample_lattice.interpolate_value(0.29, 0.42, 0.48) == 5.0
    assert sample_lattice.interpolate_value(0.29, 0.42, 0.56) == 0.0
...
def test_extract_slice(sample_lattice):
    ...
    compare_data = np.array([5.0] + 99 * [0.0])
...
def test_extract_slice_to_csv(sample_lattice, tmp_path):
    ...
    with open(filename, "r") as f:
        ...
        headers = next(reader)
        expected_headers = [
            "#Slice in Y-Z Plane at X = 0.2222222222222222. Ymin: 0 Ymax: 1 ny: 10 Zmin: 0 Zmax: 1 nz:10 "
        ]
...
def test_interpolate_to_lattice(sample_lattice):
    ...
    # Assume set_value_by_index and interpolate_value methods are properly
    # defined
    for i in range(10):
        for j in range(10):
            for k in range(10):
...
def test_interpolate_to_lattice_to_new_extent(sample_lattice):
    ...
    # Call interpolate_to_lattice
    new_lattice = lattice.interpolate_to_lattice_new_extent(
        20, 20, 20, -1, 1, -1, 1, -1, 1
    )
...
def test_add_particle_data(
    ...
    with pytest.raises(ValueError):
        sample_lattice.add_particle_data(
            ...
            kernel="Commander Riker",
            add=False,
        )

    # Add the particle data to the lattice
    sample_lattice.add_particle_data(
        particle_list_strange,
        sigma=1e-2,
        ...
        for j in range(10):
            for k in range(10):
                if i == 0 and j == 0 and k == 0:
                    assert np.isclose(
                        sample_lattice.get_value_by_index(i, j, k),
                        1.0 / sample_lattice.cell_volume_,
                    )
                if i == 5 and j == 5 and k == 5:
                    assert np.isclose(
                        sample_lattice.get_value_by_index(i, j, k),
                        2.0 / sample_lattice.cell_volume_,
                    )
        ...
        for j in range(10):
            for k in range(10):
                ...
                if i == 5 and j == 5 and k == 5:
                    assert np.isclose(
                        sample_lattice.get_value_by_index(i, j, k),
                        4.0 / sample_lattice.cell_volume_,
                    )
    ...
    assert np.isclose(
        sample_lattice.get_value_nearest_neighbor(0.444, 0.444, 0.444)
        / sample_lattice.get_value_nearest_neighbor(0.555, 0.555, 0.555),
        5.5381e-2,
    )
    # from definition of mulivariate normal distribution
    assert np.isclose(integral * sample_lattice.cell_volume_, 10.0)
    sample_lattice.add_particle_data(
        particle_list_center,
        sigma=1e-1,
        quantity="energy_density",
