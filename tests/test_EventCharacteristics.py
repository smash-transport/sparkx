...
def test_EventCharacteristics_initialization(
    ...
    assert np.array_equal(
        eve_char.event_data_.grid_,
        test_Lattice3D_instance.grid_)
...
def test_eBQS_densities_Milne_from_OSCAR_IC(
    ...
    assert all(isinstance(line, str)
               for line in header_info), "Not all lines of the header are strings"
    ...
    for line in data_lines:
        ...
        tau, x, y, eta, energy_density, baryon_density, charge_density, strangeness_density = map(
            float, values)
        ...
        integral += energy_density * tau * \
            (x_max - x_min) * (y_max - y_min) * (eta_range[1] - eta_range[0]) / (Nx * Ny * eta_range[2])
    assert np.isclose(
        integral, 2., atol=1e-1), "Energy density integral is not close to 2"
