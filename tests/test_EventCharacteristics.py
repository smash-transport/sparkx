...
def test_Lattice3D_instance():
    ...
    lattice.set_value_nearest_neighbor(0.0, 1.0, 0.0, 1.0)
    lattice.set_value_nearest_neighbor(0.0, -1.0, 0.0, 1.0)
    return lattice


def test_EventCharacteristics_initialization(
    oscar_particle_objects_list, test_Lattice3D_instance
):
...
def test_eccentricity_from_particles(fake_particle_list_eccentricity):
    ...
    assert np.allclose(eps2, (1.0 - 0j), atol=1e-10)

    eps3 = eve_char.eccentricity(3, weight_quantity="number")
    assert np.allclose(eps3, (0.0 - 0j), atol=1e-10)
...
def test_eccentricity_from_lattice(test_Lattice3D_instance):
    eve_char = EventCharacteristics(test_Lattice3D_instance)
    eps2 = eve_char.eccentricity(2, weight_quantity="number")
    np.allclose(eps2, (1.0 - 0j), atol=1e-10)

    eps3 = eve_char.eccentricity(3, weight_quantity="number")
    np.allclose(eps3, (0.0 - 0j), atol=1e-10)
...
def test_eBQS_densities_Milne_from_OSCAR_IC(
    tmp_path,
    oscar_particle_objects_list,
    test_Lattice3D_instance,
    fake_particle_list_eccentricity,
    initial_particles,
):
    ...
    assert all(
        isinstance(line, str) for line in header_info
    ), "Not all lines of the header are strings"
    ...
    assert np.isclose(
        integral, 2.0, atol=1e-1
    ), "Energy density integral is not close to 2"


def test_eBQS_densities_Minkowski_from_OSCAR_IC(
    tmp_path, oscar_particle_objects_list, test_Lattice3D_instance
):
    ...
    kernel = "covariant"
    output_filename = os.path.join(
        tmp_path, "test_eBQS_densities_Minkowski_from_OSCAR_IC.dat"
    )
